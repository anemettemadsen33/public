...
export const usePreferencesStore = create(
  persist(
    (set, get) => ({
      ...
      addViewedVehicle: (vehicleId) => set((state) => ({
        viewedVehicles: [
          vehicleId,
          ...state.viewedVehicles.filter(id => id !== vehicleId)
        ].slice(0, 50) // Keep last 50 views
      })),
      
      trackFilterClick: (filterType, value) => set((state) => ({
        clickedFilters: {
          ...state.clickedFilters,
          [filterType]: [
            ...(state.clickedFilters[filterType] || []),
            value
          ].slice(-10) // Keep last 10 per filter type
        }
      })),
      
      saveVehicle: (vehicle) => set((state) => ({
        savedVehicles: [
          vehicle,
          ...state.savedVehicles.filter(v => v.id !== vehicle.id)
        ]
      })),
      
      unsaveVehicle: (vehicleId) => set((state) => ({
